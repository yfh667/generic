...
def _worker_one_range(ttb: int, start_ts: int, end_ts: int, pkl_path: str, out_dir: str) -> str:
    ...
    # 由于我们没有建链时间的约束,因此,这个求出来的raw_inter_edge就是事实上的拓扑


    realnodes = inter_edge2nodes.trans_edge2node(raw_inter_edge, cfg.P, cfg.N)
    ...
    print(f"raw({start_ts},{end_ts}) -> {out_path}")
    return str(out_path)
