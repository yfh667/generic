...
import draw.pymatlab2.basic.merge_pendinglinks as merge_pendinglinks
...
def run_one_ttb(ttb: int) -> tuple[int, list[str]]:
    ...
    merged_pending_edge = merge_pendinglinks.merge_edges_by_step(pending_edge, iG_edge)

    out_paths1 = psn.export_pending_series_to_origin(
        pending_edge,
        out_dir=figure_dir,
    ...
    out_paths2 = psn.export_pending_series_to_origin(
        iG_edge,
        out_dir=figure_dir,
        basename=f"iG_edge_ttb{ttb}",
        to=("csv",),          # 需要 Excel 同时导出可改为 ("csv","xlsx")
        fill_missing=True,
        max_t_seconds=None,
        smooth_window=None    # 想多导一列平滑曲线就填窗口大小（如 5）
    ...
    dt = time.time() - t0
    out_paths = [*out_paths1, *out_paths2]  # 拼起来
    out_paths_str = [str(p) for p in out_paths]  # 转成 str
    print(f"[TTB={ttb}] 完成，用时 {dt:.1f}s -> {', '.join(out_paths_str)}")
    return ttb, out_paths_str
