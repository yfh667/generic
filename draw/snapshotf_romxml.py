...
def extract_region_satellites_from_file(xml_file_path, start_ts, end_ts):
    """
    ...
    # Initialize a list to store the region paper_dataresult for each time step
    result_lists = []
    ...
    try:
        ...
        # Iterate through all time steps in the XML
        for time_snap in root.findall('time'):
            time_step = int(time_snap.get('step'))  # Get the time step from the XML attribute
            if start_ts <= time_step <= end_ts:  # Check if the time step is in the specified range
                # Initialize a list of sets for the current time step
                region_satellites = [set() for _ in range(len(regions_map))]

                stations_element = time_snap.find('stations')
                if stations_element is not None:
                    # Iterate through each station in the time step
                    for station_element in stations_element.findall('station'):
                        station_id_str = station_element.get('id')
        ...
        if not result_lists:
            print(f"Warning: Time steps {start_ts} to {end_ts} not found in the XML data.")

    except FileNotFoundError:
        print(f"Error: XML file not found at path '{xml_file_path}'")
        # Return empty lists if file not found
        result_lists = [[[] for _ in range(len(regions_map))] for _ in range(end_ts - start_ts + 1)]
    except ET.ParseError as e:
        print(f"Error parsing XML file '{xml_file_path}': {e}")
        # Return empty lists if parsing fails
        result_lists = [[[] for _ in range(len(regions_map))] for _ in range(end_ts - start_ts + 1)]
