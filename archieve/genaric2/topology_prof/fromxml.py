...
def main(individual_file):
    ...
    # 2D 动态图可视化
    adjacency_list_array = action_table.action_map2_shanpshots(individual1, P, N, T)
    ...
    for t in range(T):
        ...
        for i in range(P):
            for j in range(N):
                key = i * N + j
                value = individual1[(i, j, t)].importance
                weight[key] = value
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Visualize basicSa topology_prof and regions.")
