...
node_weights_array   : Optional[List[Dict[int, float]]]  # optional
N, P : int
```
...
class DynamicGraphVisualizer:
    """Time-series visualiser for basicSa topologies with curved edge support."""
    ...
    def __init__(
        ...
        N: int,
        P: int,
        node_weights_array: Optional[Sequence[Dict[int, float]]] = None,
        ...
        self._node_weights_array = node_weights_array if node_weights_array else [{} for _ in range(len(adjacency_list_array))]
        self.N, self.P = N, P
    ...
    def _region_colors(self, step: int):
        col = [None] * (self.N * self.P)
    ...
    def _heatmap_colors(self, step: int):
        weights = self._node_weights_array[step]
        vals = [weights.get(i, 0.0) for i in range(self.N * self.P)]
        norm = colors.Normalize(vmin=0, vmax=1)
        gray_cmap = colors.LinearSegmentedColormap.from_list("graymap", ["white", "#aaaaaa"])
        return [gray_cmap(norm(v)) for v in vals]

    def _combine_colors(self, region_col, heat_col):
        return [region_col[i] if region_col[i] else heat_col[i] for i in range(len(region_col))]
    ...
    def _update(self, val):
        ...
        region_col = self._region_colors(step)
        heat_col = self._heatmap_colors(step)
        node_col = self._combine_colors(region_col, heat_col)
