...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Lazy 2D viewer for three fixed motifs. Edge usage is computed only "
            "for the currently displayed time step, not precomputed for all steps."
        )
    )
...
def make_lazy_edge_usage_provider(*, spec: TopologySpec, config, pair: RegionPair, group_data: dict):
    adjacency = build_undirected_adjacency(spec.edge_table, int(config.total_sats))
    state_cache: dict[tuple[tuple[int, ...], tuple[int, ...]], tuple[object, object]] = {}
    ...
    def provider(_row: int, step: int):
        ...
        state_key = (source_nodes, target_nodes)
        ...
        status = (
            f"lazy edge usage | states cached {len(state_cache)} | "
        ...
        *,
        ...
        for spec in specs:
            ...
            viewer = LazyEdgeUsageTopology2DViewer(
                ...
                edge_usage_provider=make_lazy_edge_usage_provider(
                    spec=spec,
                    config=config,
                    pair=pair,
                    group_data=group_data,
                ),
                window_title=f"{pair.label} lazy edge usage | {spec.name}",
                group_data=group_data,
...
class MultiPairLazyWindow(QtWidgets.QWidget):
    def __init__(self, *, config_name: str, tabs: list[tuple[RegionPair, PairTab]]):
        super().__init__()
        self.setWindowTitle(f"{config_name} lazy fixed motif edge-usage comparison")
        layout = QtWidgets.QVBoxLayout(self)
        layout.setContentsMargins(8, 8, 8, 8)
...
def main() -> int:
    ...
    print(
        f"[lazy-edge-usage-viewer] no full edge-usage precompute. "
    ...
    for spec in selected_specs:
        print(
            f"[lazy-edge-usage-viewer] topology={spec.name}, motif={spec.motif}, edges={spec.edge_table.num_edges}",
            flush=True,
        )
    ...
    window = MultiPairLazyWindow(config_name=config.name, tabs=tabs)
