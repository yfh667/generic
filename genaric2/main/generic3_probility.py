    ...
    for generation in range(generations):
        ...
        for i in range(0, len(crossarrary), 2):
            if (i != len(crossarrary) - 1):
                ...
                flag1, connection1_test, connection2_test = TopoSeqValidator.TologialSequenceValidator(parent1, P, N, T,
                                                                                                       regions_to_color,
                                                                                                       setuptime)

                flag2, connection1_test, connection2_test = TopoSeqValidator.TologialSequenceValidator(parent2, P, N, T,
                                                                                                       regions_to_color,
                                                                                                       setuptime)
                if  flag1==0 or flag2==0:
                    print(" para cuowu ")
                    print(f"para point{point} ")
                ...
                flag1, connection1_test, connection2_test = TopoSeqValidator.TologialSequenceValidator(child1, P, N, T,regions_to_color,
                                                                                                       setuptime)

                flag2, connection1_test, connection2_test = TopoSeqValidator.TologialSequenceValidator(child2, P, N, T,regions_to_color,
                                                                                                       setuptime)
                if  flag1==0 or flag2==0:
                    print(" zajiao cuowu ")
                ...
                #     # 保存关键数据（确保即使验证崩溃也能保存）
                #     writetoxml.nodes_to_xml(child1, "E:\\code\\paper_dataresult\\2\\child1_debug.xml")
                #     writetoxml.nodes_to_xml(child2, "E:\\code\\paper_dataresult\\2\\child2_debug.xml")
                #     writetoxml.nodes_to_xml(parent1, "E:\\code\\paper_dataresult\\2\\parent1_debug.xml")
                #     writetoxml.nodes_to_xml(parent2, "E:\\code\\paper_dataresult\\2\\parent2_debug.xml")
                #
                #     # 打印错误信息
        ...
        for i in next_population:
            flag1, connection1_test, connection2_test = TopoSeqValidator.TologialSequenceValidator(i, P, N, T,  regions_to_color,   setuptime)
            if  flag1==0:
                print(" cross cuowu ")
                os._exit(1)
                try:
        ...
        for i in range(len(mutate_arrary)):
          ...
          #  if random.random() < 0.5:  # 50%的概率
            mutate_individual, mutate_node,chose,type = mutate.mutate_f(individual, regions_to_color, inter_link_bandwidth,
            ...
            if mutate_individual != individual:
                flag1, connection1_test, connection2_test = TopoSeqValidator.TologialSequenceValidator(
                    mutate_individual, P, N, T, regions_to_color,setuptime)

                if flag1 == 0:
                    print("mutate cuowu ")
                    print(f"mutate node {mutate_node},neighbor ={chose} type is {type}")
...
# 绘制适应度历史曲线
reciprocal_history = [ fitness for fitness in fitness_history]
plt.figure()
