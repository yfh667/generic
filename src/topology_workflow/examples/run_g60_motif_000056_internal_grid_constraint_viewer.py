...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Draw G60 motif_000056 with selected-region internal +grid constraints."
    )
...
def main() -> int:
    ...
    meta = {
        ...
        "rule": (
            ...
            "For selected-group internal inter-plane pairs, force option 0 (+grid) links. "
            "If a selected-region node side has such an internal link, remove other inter-plane links "
            "on the same side."
        ),
        **stats,
        "outputs": {
    ...
    print(
        f"[region-constraint-example] step={step} motif={MOTIF_000056['name']} "
        f"edges={stats['kept_edges']} base={stats['base_edges']} "
        f"forced_internal={stats['forced_internal_option_edges']} "
        f"dropped={stats['dropped_edges']} out={out_dir}",
        flush=True,
    )
    print(
        f"[region-constraint-example] validation={stats['validation']} "
        f"option_counts={stats['option_counts_kept']}",
        flush=True,
    )
    if args.check_only:
        return 0
