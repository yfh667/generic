...
from src.topology_learning.module.lst_schedule_eval import run_lst_evaluation_from_selector
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate a selector schedule after generic LST active/building simulation.")
    ...
    parser.add_argument(
        ...
        help="Legacy reactive setup mode. Ignored by advance timing.",
    )
    parser.add_argument(
        "--setup-timing",
        ...
        default="reactive",
        help=(
            "Default 'reactive' treats the selected topology sequence as setup commands: "
            "a new edge is building for LST seconds before it becomes active. "
            "Use 'advance' only for offline known-future upper-bound runs."
        ),
    )
