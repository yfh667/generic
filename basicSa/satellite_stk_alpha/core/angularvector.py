...
class SatelliteVector:
    ...
    def get_vectors2(self,RAAN,inclination,vectorbase,delta_current ):

        cosR = np.cos(RAAN)  # RAAN余弦
        sinR = np.sin(RAAN)  # RAAN正弦
        cosB = np.cos(inclination)  # 轨道倾角余弦
        sinB = np.sin(inclination)  # 轨道倾角正弦
       ...
       # nextposition = vecs['next']
        ...
        for i in range(7):
            ...
            for j in range(allnodeslen):
                ...
                ref_id = self._get_right_neighbor2(j,i+1)
                B_baseposition.append(baseposition[ref_id])
                B_nextposition.append(nextposition[ref_id])



            angular = self.calculate_angular_velocity_7_base(self, RAAN,inclination,baseposition[:allnodeslen],nextposition[:allnodeslen],B_baseposition,B_nextposition)

            allangulars.append(angular)



    def calculate_angular_velocity_7_base(self, RAAN,inclination,A_baseposition,A_nextposition,B_baseposition,B_nextposition):

        delta_current  = B_baseposition-A_baseposition

        current_rotated = self.get_vectors2( RAAN,inclination,A_baseposition,delta_current  )
        # 下一时刻旋转

        delta_next = B_nextposition - A_nextposition
        next_rotated = self.get_vectors2( RAAN,inclination,A_nextposition,delta_next  )
    ...
    #
    #
    #     current_rotated = self.get_vectors( vecs,vecs['base'],vecs['delta_current']  )
    #     # 下一时刻旋转
    #
    #
    #     next_rotated = self.get_vectors( vecs,vecs['next'],vecs['delta_next']  )
    #
    #     # =============================
    #     # 4. 角速度计算
    #     # =============================
...
# use the eci vector raw it is wrong
#     def calculate_angular_velocity(self,time):
#         """计算卫星间相对角速度（严格三维公式实现）
#
#         返回值：
