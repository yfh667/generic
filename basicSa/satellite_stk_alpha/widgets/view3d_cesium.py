...
class CesiumSatellite3DView(QWidget):
    ...
    def __init__(self, sim, parent=None):
      ...
      #  self.sim.positions_updated.connect(self.on_positions_updated)
    ...
    def on_positions_updated(self,time, positions,gnd):
        ...
        for i in range(length):
            node = positions[i][0]
            satid = node.nodeid - 1
            # for i in range(1,len(node.linked_array)):
            #     if node.linked_array[i] != -1:
            #         node.linked_array[i] = node.linked_array[i] + gndnum
            sat_data.append({
