...
class ControlPanel(QWidget):
    ...
    # def load_trajectory_file2(self):
    #     """加载文件夹下所有txt文件"""
    ...
    #         try:
    #             # 从文件名提取sat_id
    #             sat_id = int(os.path.splitext(filename)[0])
    #         except ValueError:
    #             error_files.append(f"{filename}: 无效的卫星ID")
    #             continue
    #
    #         try:
    ...
    #                     if len(parts) != 4:
    #                         self.log.append(f"{filename} 第{line_num}行: 需要4个数值")
    #                         continue
    #
    #                     try:
    #                         time = float(parts[0])
    ...
    #                     except ValueError as e:
    #                         self.log.append(f"{filename} 第{line_num}行: 数据错误 - {str(e)}")
    #
    #                 if points:
    #                     self.manager.add_trajectory(sat_id, points)
    #                     success_count += 1
    #                 else:
    #                     error_files.append(f"{filename}: 无有效数据")
    #
    ...
    def load_trajectory_file(self):
        ...
        if not path:
            return
       # stationsnodes = readdata.readstation(path, stationangle)  # 获取地面站数据
    ...
    def load_trajectory_filenew(self):
        ...
        #         id = i*N+j
        #         sat = satsnodes[id]
        #         self.manager.add_trajectory(sat[0].nodeid, sat)
        #         RAAN = (i ) * SatelliteConfig.RAAN_INCREMENT
        #
        #    #     RAAN =(i+1)*10.2
        #         readdata.add_number_RAAN(sat, RAAN, track_angle)
        #         success_count += 1
