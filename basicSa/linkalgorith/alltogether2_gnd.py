  ...
  #  los_mask = angular_vel < settings.SatelliteConfig.ANGULAR_VELOCITY
   ...
   # gndlink = np.zeros(len(gndnodes), dtype='int32')
    ...
    for target in targets:
        ...
        for sat in start_satellites:
            if small>sat[1]:
                small = sat[1]
                start_satellite = sat[0]


        small = 1
        end_satellite = None
        for sat in end_satellites:
            if small>sat[1]:
                small = sat[1]
                end_satellite = sat[0]
    ...
    if min_path:
        ...
        min_paths.append([target[0]]+new_path+[target[1]])
        if time == 62:
            print(f"min_path is {min_path}")
