...
STATION_GROUPS = {
    0: {"name": "Group 0", "stations": list(range(0, 9))},
    1: {"name": "Group 1", "stations": [9]},
    2: {"name": "Group 2", "stations": [10]},
    3: {"name": "Group 3", "stations": list(range(11, 15))},
    4: {"name": "Group 4", "stations": list(range(15, 17))},
    5: {"name": "Group 5", "stations": list(range(17, 20))},
}
    ...
    for time_elem in root.findall('time'):
        ...
        for station_elem in stations_elem.findall('station'):
            ...
            if assigned_gid is not None:
                 # 提取该地面站可见的卫星ID并转换为整数
                sat_ids_str = [sat.get('id') for sat in station_elem.findall('basicSa')]
                # 过滤掉None值，并尝试转换为整数
                valid_sat_ids = set()
                for sat_id_str in sat_ids_str:
                    if sat_id_str is not None:
                         try:
                             ...
                             print(f"Warning: Could not convert basicSa ID '{sat_id_str}' to integer at step {step}, station {station_id}")
...
if __name__ == "__main__":
    xml_file = "/home/yfh/Desktop/Data/1/station_visible_satellites_648.xml"  # <<<<<<< 请替换为你的XML文件路径 >>>>>>>

    try:
        # 解析XML数据
        group_data = parse_xml_group_data(xml_file)

        if not group_data:
            print(f"Error: No valid group visibility data parsed from {xml_file}.")
            print("Please check if the XML file exists and contains 'time' elements with 'stations' and 'basicSa' data.")
